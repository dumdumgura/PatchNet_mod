...
class ShapeNet(Dataset):
    ...
    def __getitem__(self, idx):
        if self.type == 'sdf' or self.type =='occ':
            ...
            #near_surface_pts = point_cloud[pts//3:pts*2//3,:]
            #uniform_pts = point_cloud[pts*2//3:pts, :]
            ...
            #uniform_sample = self.create_sdf_sample(uniform_pts)
            #uniform_sample = uniform_pts[np.random.choice(uniform_pts.shape[0], size=self.batch // 2)]
            on_surface_sample = on_surface_pts[np.random.choice(on_surface_pts.shape[0], size=self.batch//2)]
            #self.vis_data(uniform_sample)
            #self.vis_data(on_surface_sample)


            #sample_ns = np.concatenate([near_surface_sample,uniform_sample],axis=0)
            sample_os = np.concatenate([on_surface_sample], axis=0)

            sample_os[:, 3] = np.zeros_like(sample_os[:,3])
            #self.vis_data(sample_os)

            #sample_os[:self.batch//2,3] = 0  #on surface
            sample_os[self.batch//2:,3] = -1 #off_surface

            #self.vis_data(sample_ns)
            #self.vis_data(sample_os[:self.batch//2,:])

            return {#"near_surface_pts": torch.from_numpy(sample_ns).float(),
                    "on_surface_pts": torch.from_numpy(sample_os).float(),
                    #"grid": torch.from_numpy(grid_pts).float(),
                    "idx": idx
                    }
