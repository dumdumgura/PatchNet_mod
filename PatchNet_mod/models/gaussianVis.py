...
if __name__ == '__main__':
    P = torch.randn(3,20,3)
    norm = P.norm(dim=-1)
    mask = norm>1
    sel_P = P[mask]

    # Specify the dimension along which to split (e.g., num_pts dimension)
    split_dimension = 1
    # Number of splits (you can adjust this based on your requirement)
    num_splits = 5

    # Use torch.split to split the tensor along the specified dimension
    split_tensors = torch.split(P, split_size_or_sections=num_splits, dim=split_dimension)

    # Convert the split_tensors into a list
